...
template <class T>
class cubic_logic_parser_class {
public:
    // Initialise or define the parser categories of tokens
    cubic_logic_parser_class() {
        define_white_space();
        define_braces();
        ...
        define_cubic_grid_neighbour_tokens();
    }

    ~cubic_logic_parser_class() {}

    // for testing only Delete or comment out when not in use or needed
    //cubic_grid_base_class<T> *cubic_test_grid = nullptr;

    //cubic_grid_value_data_type_enum cubic_grid_data_type = cubic_grid_value_data_type_enum::Integer;
    ...
    parser_base_node* create_logic_parse_tree(std::string& text) {
        ...
        /*
        if (!cubic_test_grid) {
            printf("logic_parser_class::Parse  cubic_test_grid is nullptr : cannot test get hex grid neighbours \n");
            return nullptr;
        }


        size_t H0_index = 75;
        cubic_test_grid->cubic_grid[H0_index] = 0;
        printf("logic_parser_class::Parse  test grid neighbour 0 grid index %i : value |%i|\n", H0_index, cubic_test_grid->cubic_grid[H0_index]);

        cubic_surface_index_data_type h_i = get_cubic_neighbour_index<T>(cubic_test_grid, H0_index, 1);
        if (h_i > -1) { cubic_test_grid->cubic_grid[h_i] = 1; }
        printf("logic_parser_class::Parse  test grid neighbour 1 grid index %i : value |%i|\n", h_i, cubic_test_grid->cubic_grid[h_i]);
        h_i = get_cubic_neighbour_index(cubic_test_grid, H0_index, 2);
        if (h_i > -1) { cubic_test_grid->cubic_grid[h_i] = 2; }
        printf("logic_parser_class::Parse  test grid neighbour 2 grid index %i : value |%i|\n", h_i, cubic_test_grid->cubic_grid[h_i]);
        h_i = get_cubic_neighbour_index(cubic_test_grid, H0_index, 3);
        if (h_i > -1) { cubic_test_grid->cubic_grid[h_i] = 3; }
        printf("logic_parser_class::Parse  test grid neighbour 3 grid index %i : value |%i|\n", h_i, cubic_test_grid->cubic_grid[h_i]);
        h_i = get_cubic_neighbour_index(cubic_test_grid, H0_index, 4);
        if (h_i > -1) { cubic_test_grid->cubic_grid[h_i] = 4; }
        printf("logic_parser_class::Parse  test grid neighbour 4 grid index %i : value |%i|\n", h_i, cubic_test_grid->cubic_grid[h_i]);
        h_i = get_cubic_neighbour_index(cubic_test_grid, H0_index, 5);
        if (h_i > -1) { cubic_test_grid->cubic_grid[h_i] = 5; }
        printf("logic_parser_class::Parse  test grid neighbour 5 grid index %i : value |%i|\n", h_i, cubic_test_grid->cubic_grid[h_i]);
        h_i = get_cubic_neighbour_index(cubic_test_grid, H0_index, 6);
        if (h_i > -1) { cubic_test_grid->cubic_grid[h_i] = 6; }
        printf("logic_parser_class::Parse  test grid neighbour 6 grid index %i : value |%i|\n", h_i, cubic_test_grid->cubic_grid[h_i]);


        //evaluate_parse_tree(t_node, cubic_test_grid, H0_index);
    ...
    bool evaluate_parse_tree(parser_base_node* tt_node, cubic_grid_base_class<T>* cubic_grid, cubic_surface_index_data_type cubic_index) {
        if (!tt_node) {
        ...
        case function_type_enum::binary: {
            binary_node* bn = static_cast<binary_node*>(tt_node);
            if (!evaluate_parse_tree(bn->_leftExpression, cubic_grid, cubic_index)) {
            ...
            if (!evaluate_parse_tree(bn->_rightExpression, cubic_grid, cubic_index)) {
        ...
        case  function_type_enum::data: {
            literal_node* ln = static_cast<literal_node*>(tt_node);
            if (!functions.get_cubic_grid_value(ln, cubic_grid, cubic_index)) {
...
//printf("logic_parser_class :: ParseExp ERROR :Expected ')'\n", tokens[index].c_str());
    ...
    parser_base_node* ParseSubExp(const std::vector<std::string>& tokens, size_t& index) {
        ...
        if (have_cubic_grid_neighbour(token)) {// get hex grid cell data value and assign to the value of a paser literal node 
            index++;// next parser token index
...
private:
    ...
    cubic_functions_class functions;
    ...
    std::vector<std::string> cubic_grid_neighbours;
    ...
    void define_cubic_grid_neighbour_tokens() {
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C0);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C1);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C2);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C3);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C4);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C5);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C6);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C7 );
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C8 );
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C9 );
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C10);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C11);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C12);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C13);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C14);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C15);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C16);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C17);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C18);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C19);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C20);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C21);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C22);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C23);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C24);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C25);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C26);
        cubic_grid_neighbours.push_back(CUBIC_GRID_NEIGHBOUR_C27);
    }
    ...
    bool have_cubic_grid_neighbour(std::string token) {
        for (std::string cubic_grid_neighbour : cubic_grid_neighbours) {
            if (token == cubic_grid_neighbour) { return true; }
        }
        return false;
    }
};