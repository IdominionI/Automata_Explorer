...
#include "Source/Cubic/Cubic_grids/cubic_grid_parameters.h"
...
struct cubic_automata_sub_rule_struct_type {
...
struct cubic_surface_automata_rule_struct_type {
	...
	std::vector<cubic_automata_sub_rule_struct_type> sub_rules;// List of sub rules that make up this rule defied as a dynamic vector

	// !!!!!!!!! Functions to manage the sub rule definitions that make up a rule !!!!!!!!!!

	void add_sub_rule(cubic_automata_sub_rule_struct_type sub_rule) {
	...
	cubic_automata_sub_rule_struct_type *get_sub_rule(int sub_rule_id) {
		for (cubic_automata_sub_rule_struct_type sub_rule : sub_rules) {
	...
	bool modify_sub_rule(cubic_automata_sub_rule_struct_type sr) {
		for (cubic_automata_sub_rule_struct_type sub_rule : sub_rules) {
...
struct cubic_surface_automata_rule_result_struct_type {
	...
	std::string cubic_result_definition = "";			// Text based definition of which rule(s) are to be met
	std::string cubic_result_value      = "";			// Result to assign hex grid automata cell if all rule conditions are met
	...
	std::vector<int> cubic_surface_automata_rules_met;// Indicate if all rule results have been met or not for the current iteration step
...
//struct cubic_surface_automata_generator_parameters_struct_type {
