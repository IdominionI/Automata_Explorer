...
public:
    ...
    hcp_logic_parser_class() {
        ...
        define_hcp_grid_neighbour_tokens();
    }
    ...
    //hcp_grid_value_data_type_enum hcp_grid_data_type = hcp_grid_value_data_type_enum::Integer;
...
//printf("logic_parser_class :: delete_pars_tree completed\n");
    ...
    bool evaluate_parse_tree(parser_base_node* tt_node, hcp_grid_base_class<T>* hcp_grid, hcp_surface_index_data_type hcp_index) {
        if (!tt_node) {
        ...
        case function_type_enum::binary: {
            binary_node* bn = static_cast<binary_node*>(tt_node);
            if (!evaluate_parse_tree(bn->_leftExpression, hcp_grid, hcp_index)) {
//              printf("evaluate_parse_tree ERROR : Left expression value cannot be evaluated %s\n", bn->_leftExpression->function_token_definition.second.c_str());
            ...
            if (!evaluate_parse_tree(bn->_rightExpression, hcp_grid, hcp_index)) {
...
//display_evaluation_value(bn);// testing only : delete comment out when not needed
        ...
        case  function_type_enum::data: {
            literal_node* ln = static_cast<literal_node*>(tt_node);
            if (!functions.get_hcp_grid_value(ln, hcp_grid, hcp_index)) {
...
//printf("logic_parser_class :: ParseSubExp unary_node 2222 %s\n", u_node->function_token_definition.second.c_str());
        ...
        if (have_hcp_grid_neighbour(token)) {// get hex grid cell data value and assign to the value of a paser literal node 
            index++;// next parser token index
...
private:
    ...
    std::vector<std::string> hcp_grid_neighbours;
    ...
    void define_hcp_grid_neighbour_tokens() {
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H0);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H1);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H2);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H3);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H4);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H5);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H6);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H7);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H8);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H9);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H10);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H11);
        hcp_grid_neighbours.push_back(HCP_GRID_NEIGHBOUR_H12);
    }
    ...
    bool have_hcp_grid_neighbour(std::string token) {
        for (std::string hcp_grid_neighbour : hcp_grid_neighbours) {
            if (token == hcp_grid_neighbour) { return true; }
        }
        return false;
    }
};