...
/*
 ...
 Effekseer/ImGradientHDR that has additional dependency header files

<FrameWork/AFW/Tools/afw_dialogs.h>
<FrameWork/AFW/Tools/afw_file_tools.h>
...
//ImDrawList::AddText(ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
...
bool ImGradientHDR(int32_t gradientID, ImGradientHDRState& state, ImGradientHDRTemporaryState& temporaryState, bool isMarkerShown)
{
	...
	if (isMarkerShown)
	{
		...
		if (ImGui::IsItemHovered())
		{
			...
			float grad_value = ((float)state.max_grad_value - (float)state.min_grad_value) * xn;
			state.current_gradient_position = xn;
			state.current_gradient_value = grad_value;
			state.current_gradient_color = c;
...
//printf("ImGradientHDR :: color pos : %f3 : color %f.2 | %f.2 | %f.2 | %f.2 |\n",xn, c[0], c[1], c[2],c[3]);
//printf("ImGradientHDR :: x-value : %f3 | grad value : %f3 |\n",xn, grad_value);
			}
...
bool export_gradient(std::string file_pathname, ImGradientHDRState state) {
	...
	for (int i = 0; i < state.ColorCount; i++) {
		printf("Hex_grid_class::Save Gradient : %i : %4.2f | %4.2f | %4.2f |  \n", i, state.Colors[i].Color[0], state.Colors[i].Color[1], state.Colors[i].Color[2]);
	}
	...
	if (state.ColorCount < 1) {
		vwDialogs::display_error_message("Export Gradient", "ERROR : Export Gradient :: No Gradient data defined to export");
		return false;
	}
...
void export_gradient_data(ImGradientHDRState state) {
	...
	stream << state.ColorCount << std::endl;

	for (size_t i = 0; i < state.ColorCount; i++) {
		stream << state.Colors[i].Position << ":" << state.Colors[i].Intensity << ":" << state.Colors[i].Color[0] << ":" << state.Colors[i].Color[1] << ":" << state.Colors[i].Color[2] << std::endl;
	}
}
...
bool import_gradient(std::string file_pathname, ImGradientHDRState &state) {
	...
	if (!read_gradient_data(lines, line_number, state)) {
...
bool read_gradient_data(std::vector<std::string> lines, int &line_number, ImGradientHDRState &state) {
	...
	int number_colors = std::stoi(line);// potential for crash here : need a try statement ???

	if (number_colors < 1) {
		std::string error_message = "ERROR :: Import Gradient failed\n No color data defined to create color gradient\n";
		vwDialogs::display_error_message("Import Gradient", error_message);
		return false;
	}

	std::vector<std::vector<std::string>> grad_color_data = {};

	// Add gradient color data to a vector of gradient color data before overriting existing gradient
	// This is done so if there is an error in the imported data, the existing gradient is not lost
	line_number++;
	for (size_t i = 0; i < number_colors; i++) {
		std::string line = lines[line_number];
		std::vector<std::string> color_data_str = FW::stringtools::split(line, ":");
		
		if (color_data_str.size() != 5) {
			std::string error_message = "ERROR :: Import Gradient failed\n corrupted or out of sequence data at line\n";
			error_message += std::to_string(line_number) + "\n with entry  \n" + lines[line_number] + "\n";
			vwDialogs::display_error_message("Import Gradient", error_message);
			return false;
		...
		grad_color_data.push_back(color_data_str);

		line_number++;
	}

//printf("ImGradientHDR::read_gradient_data 000: "); printf("%i\n", state.ColorCount);
	// imported gradient read correctly. Clear existing gradient color data
	for (int32_t i = state.ColorCount-1; i > -1 ; i--) {
		state.RemoveColorMarker(i);
	}
//printf("ImGradientHDR::read_gradient_data 111: "); printf("%i\n", state.ColorCount);
	//Add imported colors to gradient
	for (size_t i = 0; i < grad_color_data.size(); i++) {
		ImGradientHDRState::ColorMarker color_data;

		std::vector<std::string> color_data_str = grad_color_data[i];

		color_data.Position  = stof(color_data_str[0]);// potential for crash here : need a try statement ???
		color_data.Intensity = stof(color_data_str[1]);
		color_data.Color[0]  = stof(color_data_str[2]);
		color_data.Color[1]  = stof(color_data_str[3]);
		color_data.Color[2]  = stof(color_data_str[4]);

		state.AddColorMarker(color_data.Position, color_data.Color,color_data.Intensity);
	}

//printf("ImGradientHDR::read_gradient_data 222: "); printf("%i\n", state.ColorCount);
	return true;
}
