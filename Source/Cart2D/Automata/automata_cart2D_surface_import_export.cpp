...
#include <Framework/AFW/Tools/afw_dialogs.h>// Seems cannot have or use this in a .h file. Get already has a body compile errors
	...
	int import_export_cart2D_surface_automata_rules_class::open_file_stream(std::string filename, int write_mode) {
	...
	void import_export_cart2D_surface_automata_rules_class::close_file_stream() {
		stream.close();
	}

	bool import_export_cart2D_surface_automata_rules_class::export_cart2D_surface_automata_rules(std::string file_pathname) {
		...
		write_cart2D_grid_result_rules(stream);
	...
	void import_export_cart2D_surface_automata_rules_class::write_cart2D_grid_result_rules(std::fstream& stream) {

		for(cart2D_surface_automata_rule_result_struct_type result_rule : cart2D_surface_automata_result_rules){
			...
			stream << result_rule.cart2D_result_definition << std::endl;
			stream << result_rule.cart2D_result_value		<< std::endl;
	...
	void import_export_cart2D_surface_automata_rules_class::write_result_rule_automata_rules(std::fstream& stream){
	...
	bool import_export_cart2D_surface_automata_rules_class::save_automata_rule(std::string file_pathname, cart2D_surface_automata_rule_struct_type rule) {
		if (file_pathname.size() == 0) {
	...
	bool import_export_cart2D_surface_automata_rules_class::save_automata_sub_rule(std::string file_pathname, cart2D_automata_sub_rule_struct_type sub_rule) {
		if (file_pathname.size() == 0) {
	...
	bool import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_rules(std::string file_pathname) {
		if (file_pathname.size() == 0) {
...
//																 file_pathname, //QMessageBox::Ok);
		...
		cart2D_surface_automata_result_rules.clear();
		cart2D_surface_automata_result_rules.shrink_to_fit();
		...
		if (!import_cart2D_automata_rules(lines, line_number)) {// Importation of hex automata data failed
			stream.close();
	...
	bool import_export_cart2D_surface_automata_rules_class::import_cart2D_automata_rules(std::vector<std::string> lines, int& line_number) {
		std::string line;
...
//std::cout <<"Import Automata byte rules", "File read Error : Read error at line " + std::string::number(line_number) + " of file \n" + file_pathname + "\nMissing START_HEADER_FLAG", //QMessageBox::Ok);
		...
		while (!FW::stringtools::contains(lines[line_number], RESULT_RULE_BLOCK_END)) {
			if (!import_cart2D_surface_automata_result_rules(lines, line_number)) {
...
//std::cout <<"Import Automata byte rules", "File read Error : Read error at line " + std::string::number(line_number) + " of file \n" + file_pathname + "\nMissing START_HEADER_FLAG", //QMessageBox::Ok);
		...
		while (!FW::stringtools::contains(lines[line_number], RULE_BLOCK_END)) {
			if (!import_cart2D_surface_automata_rules(lines, line_number)) {
				vwDialogs::display_error_message("Import hex automata rules", "ERROR : \n Could not import hex automata result rule(s)\n");
				return false;
			}
		}
...
//std::cout <<"Import Automata byte rules", "File read Error : Read error at line " + std::string::number(line_number) + " of file \n" + file_pathname + "\nMissing START_HEADER_FLAG", //QMessageBox::Ok);
	...
	bool import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_result_rules(std::vector<std::string> lines, int& line_number) {
		cart2D_surface_automata_rule_result_struct_type cart2D_surface_automata_rule_result;
		std::string line;
		line_number++;
...
//std::cout << "import_export_cart2D_surface_automata_rules_class::read_automata_byte_rule BBBBB : " << line_number << ":" << lines[line_number] << std::endl;
		cart2D_surface_automata_rule_result.result_name = line;
...
//std::cout << "import_export_cart2D_surface_automata_rules_class::read_automata_byte_rule BBBBB : " << line_number << ":" << lines[line_number] << std::endl;
		...
		if (hrule == 0){ cart2D_surface_automata_rule_result.active_result = false;} else {cart2D_surface_automata_rule_result.active_result = true;}

		line_number++;
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
		if (!FW::stringtools::string_to_int(line, &cart2D_surface_automata_rule_result.rule_start_step, error_code)) { return false; }

		line_number++;
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
		if (!FW::stringtools::string_to_int(line, &cart2D_surface_automata_rule_result.rule_end_step, error_code)) { return false; }

		line_number++;
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
		cart2D_surface_automata_rule_result.cart2D_result_definition = line;

		line_number++;
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
		cart2D_surface_automata_rule_result.cart2D_result_value = line;
		...
		cart2D_surface_automata_rule_result.result_display_color = { stof(color[0]),stof(color[1]),stof(color[2]),stof(color[3]) };

		line_number++;
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
		if (!FW::stringtools::string_to_int(line, &cart2D_surface_automata_rule_result.display_shape_id, error_code)) { return false; }
		...
		cart2D_surface_automata_result_rules.push_back(cart2D_surface_automata_rule_result);
	...
	bool import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_rules(std::vector<std::string> lines, int& line_number) {
		cart2D_surface_automata_rule_struct_type cart2D_surface_automata_rule;
		std::string line;
		line_number++;
...
//		if (!FW::stringtools::string_to_int(line, &cart2D_surface_automata_rule.rule_end_step, error_code)) { return false; }
		...
		while (!FW::stringtools::contains(lines[line_number], SUB_RULE_BLOCK_END)) {
			if (!import_cart2D_surface_automata_sub_rules(lines, cart2D_surface_automata_rule,line_number)) {
				vwDialogs::display_error_message("Import hex automata rules", "ERROR : \n Could not import hex automata result rule(s)\n");
				return false;
			}
		}

		cart2D_surface_automata_rules.push_back(cart2D_surface_automata_rule);

//std::cout << "import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_rules AAAAAAA : " << line_number << ":" << lines[line_number] << ":" << cart2D_surface_automata_rules.size() << std::endl;
	...
	bool import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_sub_rules(std::vector<std::string> lines, cart2D_surface_automata_rule_struct_type &cart2D_surface_automata_rule, int &line_number) {
		cart2D_automata_sub_rule_struct_type cart2D_automata_sub_rule;

		std::string line;
		line_number++;

		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
//std::cout << "import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_sub_rules AAAAAAA : " << line_number << ":" << lines[line_number] << std::endl;
		...
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
//std::cout << "import_export_cart2D_surface_automata_rules_class::read_automata_byte_rule BBBBB : " << line_number << ":" << lines[line_number] << std::endl;
		cart2D_automata_sub_rule.sub_rule_name = line;

		line_number++;
		line = lines[line_number];
//std::cout << "import_export_cart2D_surface_automata_rules_class::read_automata_byte_rule CCCCC : " << line_number << ":" << lines[line_number] << std::endl;
		bool hrule;
		if (!FW::stringtools::string_to_bool(line, &hrule, error_code)) { return false; }
		if(hrule == 0) {cart2D_automata_sub_rule.active_sub_rule = false;} else {cart2D_automata_sub_rule.active_sub_rule = true;}

		//next line(s) between SUB_RULE_DEFINITION_START and SUB_RULE_DEFINITION_END flags define the sub rule code

		line_number++;
		line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
//std::cout << "import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_rules AAAAAAA : " << line_number << ":" << lines[line_number] << std::endl;
		// next line read must be a SUB_RULE_START flag
		...
		while (!FW::stringtools::contains(lines[line_number], SUB_RULE_DEFINITION_END)) {
			cart2D_automata_sub_rule.sub_rule_definition += line;
			line_number++;
			line = lines[line_number]; line = FW::stringtools::truncate(line, line.size());
		}
		...
		cart2D_surface_automata_rule.sub_rules.push_back(cart2D_automata_sub_rule);
	...
	bool import_export_cart2D_surface_automata_rules_class::import_automata_rule(std::string file_pathname, std::vector <cart2D_surface_automata_rule_struct_type> &cart2D_surface_automata_rules) {
		if (file_pathname.size() == 0) {
...
//std::cout << "import_export_cart2D_surface_automata_rules_class::import_cart2D_surface_automata_rules AAAAAAA : " << line_number << ":" << lines[line_number] << ":" << cart2D_surface_automata_rules.size() << std::endl;
	...
	bool import_export_cart2D_surface_automata_rules_class::import_automata_sub_rule(std::string file_pathname, cart2D_surface_automata_rule_struct_type& cart2D_surface_automata_rule) {
		if (file_pathname.size() == 0) {
