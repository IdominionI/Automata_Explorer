...
#include "Source/Cart2D/Cart2D_Grids/cart2D_grid_parameters.h"
...
/*
	...
	cartesian 2D 2d surface data, and export/import the
	parameter data.
*/

//typedef unsigned long int u_long_int; // Jusin case needed

// Cartesian 2D automata sub rule structure type
// To store data that defines a cartesian 2D cellula automata sub rule.
struct cart2D_automata_sub_rule_struct_type {
...
// Cartesian 2D automata rule structure type
// To store data that defines a cartesian 2D cellula automata rule.
struct cart2D_surface_automata_rule_struct_type {
	...
	bool display_sub_rules = false; // Indicate if sub rules are to be displayed as an overlay on the main cartesian 2D grid or not

	std::vector<cart2D_automata_sub_rule_struct_type> sub_rules;// List of sub rules that make up this rule defied as a dynamic vector

	// !!!!!!!!! Functions to manage the sub rule definitions that make up a rule !!!!!!!!!!

	void add_sub_rule(cart2D_automata_sub_rule_struct_type sub_rule) {
	...
	cart2D_automata_sub_rule_struct_type *get_sub_rule(int sub_rule_id) {
		for (cart2D_automata_sub_rule_struct_type sub_rule : sub_rules) {
	...
	bool modify_sub_rule(cart2D_automata_sub_rule_struct_type sr) {
		for (cart2D_automata_sub_rule_struct_type sub_rule : sub_rules) {
...
// Cartesian 2D automata rule results structure type
// To store data that defines a set of cartesian 2D cellula automata rules that assign
// a value to a cartesian 2D cellular automata grid cell
struct cart2D_surface_automata_rule_result_struct_type {
	...
	std::string  result_name = "cart2D result";		// Text label name of rule result
	...
	std::string cart2D_result_definition = "";			// Text based definition of which rule(s) are to be met
	std::string cart2D_result_value      = "";			// Result to assign cartesian 2D grid automata cell if all rule conditions are met

	// Graphical definitions used to display rule results as an overlay on the main cellula automata cartesian 2D grid
	...
	std::vector<int> cart2D_surface_automata_rules_met;// Indicate if all rule results have been met or not for the current iteration step
...
//struct cart2D_surface_automata_generator_parameters_struct_type {
