...
template <class T>
class cart2D_logic_parser_class {
public:
    // Initialise or define the parser categories of tokens
    cart2D_logic_parser_class() {
        define_white_space();
        define_braces();
        ...
        define_cart2D_grid_neighbour_tokens();
    }
    ...
    //cart2D_grid_value_data_type_enum cart2D_grid_data_type = cart2D_grid_value_data_type_enum::Integer;
    ...
    parser_base_node* create_logic_parse_tree(std::string& text) {
        ...
        /*
        if (!cart2D_test_grid) {
            printf("logic_parser_class::Parse  cart2D_test_grid is nullptr : cannot test get hex grid neighbours \n");
            return nullptr;
        }


        size_t H0_index = 75;
        cart2D_test_grid->cart2D_grid[H0_index] = 0;
        printf("logic_parser_class::Parse  test grid neighbour 0 grid index %i : value |%i|\n", H0_index, cart2D_test_grid->cart2D_grid[H0_index]);

        cart2D_surface_index_data_type h_i = get_cart2D_neighbour_index<T>(cart2D_test_grid, H0_index, 1);
        if (h_i > -1) { cart2D_test_grid->cart2D_grid[h_i] = 1; }
        printf("logic_parser_class::Parse  test grid neighbour 1 grid index %i : value |%i|\n", h_i, cart2D_test_grid->cart2D_grid[h_i]);
        h_i = get_cart2D_neighbour_index(cart2D_test_grid, H0_index, 2);
        if (h_i > -1) { cart2D_test_grid->cart2D_grid[h_i] = 2; }
        printf("logic_parser_class::Parse  test grid neighbour 2 grid index %i : value |%i|\n", h_i, cart2D_test_grid->cart2D_grid[h_i]);
        h_i = get_cart2D_neighbour_index(cart2D_test_grid, H0_index, 3);
        if (h_i > -1) { cart2D_test_grid->cart2D_grid[h_i] = 3; }
        printf("logic_parser_class::Parse  test grid neighbour 3 grid index %i : value |%i|\n", h_i, cart2D_test_grid->cart2D_grid[h_i]);
        h_i = get_cart2D_neighbour_index(cart2D_test_grid, H0_index, 4);
        if (h_i > -1) { cart2D_test_grid->cart2D_grid[h_i] = 4; }
        printf("logic_parser_class::Parse  test grid neighbour 4 grid index %i : value |%i|\n", h_i, cart2D_test_grid->cart2D_grid[h_i]);
        h_i = get_cart2D_neighbour_index(cart2D_test_grid, H0_index, 5);
        if (h_i > -1) { cart2D_test_grid->cart2D_grid[h_i] = 5; }
        printf("logic_parser_class::Parse  test grid neighbour 5 grid index %i : value |%i|\n", h_i, cart2D_test_grid->cart2D_grid[h_i]);
        h_i = get_cart2D_neighbour_index(cart2D_test_grid, H0_index, 6);
        if (h_i > -1) { cart2D_test_grid->cart2D_grid[h_i] = 6; }
        printf("logic_parser_class::Parse  test grid neighbour 6 grid index %i : value |%i|\n", h_i, cart2D_test_grid->cart2D_grid[h_i]);


        //evaluate_parse_tree(t_node, cart2D_test_grid, H0_index);
    ...
    bool evaluate_parse_tree(parser_base_node* tt_node, cart2D_grid_base_class<T>* cart2D_grid, cart2D_surface_index_data_type cart2D_index) {
        if (!tt_node) {
        ...
        case function_type_enum::binary: {
            binary_node* bn = static_cast<binary_node*>(tt_node);
            if (!evaluate_parse_tree(bn->_leftExpression, cart2D_grid, cart2D_index)) {
            ...
            if (!evaluate_parse_tree(bn->_rightExpression, cart2D_grid, cart2D_index)) {
        ...
        case  function_type_enum::data: {
            literal_node* ln = static_cast<literal_node*>(tt_node);
            if (!functions.get_cart2D_grid_value(ln, cart2D_grid, cart2D_index)) {
...
//printf("logic_parser_class :: ParseExp ERROR :Expected ')'\n", tokens[index].c_str());
    ...
    parser_base_node* ParseSubExp(const std::vector<std::string>& tokens, size_t& index) {
        ...
        if (have_cart2D_grid_neighbour(token)) {// get hex grid cell data value and assign to the value of a paser literal node 
            index++;// next parser token index
...
private:
    ...
    cart2D_functions_class functions;
    ...
    std::vector<std::string> cart2D_grid_neighbours;
    ...
    void define_cart2D_grid_neighbour_tokens() {
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N0);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N1);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N2);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N3);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N4);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N5);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N6);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N7);
        cart2D_grid_neighbours.push_back(CART2D_GRID_NEIGHBOUR_N8);
    }
    ...
    bool have_cart2D_grid_neighbour(std::string token) {
        for (std::string cart2D_grid_neighbour : cart2D_grid_neighbours) {
            if (token == cart2D_grid_neighbour) { return true; }
        }
        return false;
    }
};