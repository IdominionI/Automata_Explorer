...
public:
	...
	std::vector <hex_surface_automata_rule_result_struct_type>  hex_surface_automata_result_rules;
	std::vector <hex_surface_automata_rule_struct_type>			hex_surface_automata_rules;

	bool export_hex_surface_automata_rules(std::string file_pathname);

	bool save_automata_rule(std::string file_pathname, hex_surface_automata_rule_struct_type rule);
	bool save_automata_sub_rule(std::string file_pathname, hex_automata_sub_rule_struct_type sub_rule);

	bool import_hex_surface_automata_rules(std::string file_pathname);

	bool import_automata_rule(std::string file_pathname, std::vector <hex_surface_automata_rule_struct_type> &hex_surface_automata_rules);
	bool import_automata_sub_rule(std::string file_pathname, hex_surface_automata_rule_struct_type &hex_surface_automata_rule);

private:
	// Export functions
	int  open_file_stream(std::string filename, int write_mode);
	void close_file_stream();
	void write_hex_grid_result_rules(std::fstream& stream);
	void write_result_rule_automata_rules(std::fstream& stream);

	//Import functions
	bool import_hex_automata_rules(std::vector<std::string> lines, int& line_number);
	bool import_hex_surface_automata_result_rules(std::vector<std::string> lines, int& line_number);
	bool import_hex_surface_automata_rules(std::vector<std::string> lines, int& line_number);
	bool import_hex_surface_automata_sub_rules(std::vector<std::string> lines, hex_surface_automata_rule_struct_type& hex_surface_automata_rule, int& line_number);
